...
function showSuccessElements() {
    ...
    const starsElement = document.querySelector('.unscramble-stars');
    const hintButton = document.querySelector('.unscramble-hint-btn');
    const unscrambleMiddle = document.querySelector('.unscramble-middle');
    
    if (!onThisDayContainer) return;
    
    const currentHeight = onThisDayContainer.offsetHeight;
    const isDesktop = window.innerWidth >= 768;
    const fadeTargets = [unscrambleLabel, hintButton].filter(Boolean);

    // Prevent overlapping tweens from making the panel jump.
    gsap.killTweensOf([onThisDayContainer, unscrambleLabel, hintButton, starsElement, lettersContainer, unscrambleMiddle]);

    const tl = gsap.timeline({ defaults: { overwrite: 'auto' } });

    if (fadeTargets.length) {
        tl.to(fadeTargets, {
            duration: isDesktop ? 0.32 : 0.26,
            opacity: 0,
            ease: "power1.out"
        });
    }
    ...
    let labelStartHeight = 0;
    let labelEndHeight = 0;

    tl.add(() => {
        onThisDayContainer.style.height = currentHeight + 'px';
        onThisDayContainer.style.overflow = 'hidden';

        if (hintButton) {
            // This is part of the final layout. Remove it before measuring target.
            hintButton.style.display = 'none';
            hintButton.style.opacity = '0';
        }

        if (unscrambleLabel) {
            labelStartHeight = Math.max(unscrambleLabel.getBoundingClientRect().height, 1);
            unscrambleLabel.style.height = labelStartHeight + 'px';
            unscrambleLabel.style.overflow = 'hidden';
            unscrambleLabel.style.opacity = '0';
            unscrambleLabel.style.visibility = 'hidden';
            unscrambleLabel.style.textAlign = 'center';
            unscrambleLabel.textContent = currentDayData.event;
            labelEndHeight = Math.max(unscrambleLabel.scrollHeight, labelStartHeight);
        }
        ...
        // Measure final container height using the target label height, then restore
        // the start height so both container and label can animate together.
        if (unscrambleLabel) {
            unscrambleLabel.style.height = labelEndHeight + 'px';
        }
        targetHeight = onThisDayContainer.scrollHeight;
        if (unscrambleLabel) {
            unscrambleLabel.style.height = labelStartHeight + 'px';
        }
    });

    tl.to(onThisDayContainer, {
        duration: isDesktop ? 0.62 : 0.42,
        height: () => targetHeight + 'px',
        ease: isDesktop ? "power2.out" : "power2.inOut"
    });

    if (unscrambleLabel) {
        tl.to(unscrambleLabel, {
            duration: isDesktop ? 0.62 : 0.42,
            height: () => labelEndHeight + 'px',
            ease: isDesktop ? "power2.out" : "power2.inOut"
        }, "<");
    }

    tl.add(() => {
        // Keep explicit final height to prevent auto-height snap.
        onThisDayContainer.style.height = targetHeight + 'px';
        onThisDayContainer.style.overflow = '';

        if (unscrambleLabel) {
            unscrambleLabel.style.visibility = 'visible';
            unscrambleLabel.style.overflow = 'hidden';
            unscrambleLabel.style.height = labelEndHeight + 'px';
        }
        if (starsElement) {
