...
function handlePrizeTilesCompletion(setName) {
    ...
    if (setName === 'big') {
        ...
        prefixes = ['gold_'];
        headerText = 'THE WEEKLY $20 BILL!';
        line1Text = 'THE WEEKLY $20 BILL!';
        line2Text = 'TOP TILE BOARD COMPLETE';
    } else if (setName === 'medium') {
        // Middle grid â†’ silver_ badge
        prefixes = ['silver_'];
        headerText = 'GOLDEN BADGE';
        line1Text = 'GOLDEN BADGE UNLOCKED!';
...
function applyMode() {
    ...
    if (isBrandMode) {
        ...
        prizeTierTitles.forEach(el => {
            const tier = el.getAttribute('data-prize-tier');
            if (tier === 'gold') el.innerHTML = 'The weekly $20  Bill <img class="money-bill-inline" src="https://cdn.jsdelivr.net/gh/twitter/twemoji@14.0.2/assets/72x72/1f4b5.png" alt="dollar bill">';
            else if (tier === 'silver') el.textContent = 'golden badge';
            else if (tier === 'regular') el.textContent = 'silver badge';
        });
    }
}
...
document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'u' || e.key === 'U') {
        ...
        // Reset game data, but keep tiles + badges. Also seed completed week days with random values.
        resetGameDataKeepTilesAndBadges();
    } else if (e.key === '1') {
...
// Reset game-related localStorage but preserve tiles + badge progress.
// Also backfill completed days in the current week with random values (10-40)
// so calendar history appears filled after the reset.
function resetGameDataKeepTilesAndBadges() {
    console.log('[Reset-W] Resetting game data, preserving tiles + badges...');

    const preserveKeys = ['prizeTiles', DEFAULT_BADGE_KEY, BADGES_EARNED_KEY];
    const preserved = {};
    ...
    // Fill completed days only (before today) with random points 10..40.
    for (let i = 0; i < dayOfWeek; i++) {
        const dayDate = new Date(weekStart);
        dayDate.setDate(weekStart.getDate() + i);
        const dayKey = `${dayDate.getFullYear()}-${String(dayDate.getMonth() + 1).padStart(2, '0')}-${String(dayDate.getDate()).padStart(2, '0')}`;
        const points = 10 + Math.floor(Math.random() * 31);
        localStorage.setItem(`dailyStars_${dayKey}`, String(points));
        seededTotal += points;
    }

    localStorage.setItem('totalStars', String(seededTotal));
    localStorage.setItem(`usableStars_${getTodayKey()}`, '0');

    console.log('[Reset-W] Done. Reloading...');
    setTimeout(() => {
        location.reload();
    }, 100);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const resetDemoTrigger = document.getElementById('reset-demo-trigger');
    if (resetDemoTrigger) {
        resetDemoTrigger.addEventListener('click', () => {
            resetAllData();
        });
    }
