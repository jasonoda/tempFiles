...
function validateGameData(initialGameData, breadcrumbs, finalGameData) {
  ...
  // ---------------------------------------------------------------------------------------------
  // check coin hit positions
  // ---------------------------------------------------------------------------------------------

  for(var h=0; h<breadcrumbs.length; h++){

    for(var i=0; i<breadcrumbs[h].levelCoinHits.length; i++){

      var found = false;

      for(var j=0; j<initialGameData.allCoinPositions.length; j++){

        if( breadcrumbs[h].levelCoinHits[i][0] === initialGameData.allCoinPositions[j][0] && breadcrumbs[h].levelCoinHits[i][1] === initialGameData.allCoinPositions[j][1]){
      ...
      if(found===false){
        ...
        reasons.push("LEVEL COIN NOT FOUND "+breadcrumbs[h].levelCoinHits[i][0]+" / "+breadcrumbs[h].levelCoinHits[i][1]);
        isValid=false;
  ...
  var checkLevelPoints = 0;

  for(var i=0; i<breadcrumbs.length; i++){

    checkLevelPoints += breadcrumbs[i].levelScore;

  }

  if(checkLevelPoints!==finalGameData.scoreNoLifeBonus){

    reasons.push("LEVEL POINTS DID NOT MATCH "+checkLevelPoints+" / "+finalGameData.scoreNoLifeBonus);
    isValid=false;
  ...
  var checkCoinPoints = 0;

  for(var i=0; i<breadcrumbs.length; i++){

    checkCoinPoints += breadcrumbs[i].levelCoinPoints;

  }

  if(checkCoinPoints!==finalGameData.totalCoinPoints){

    reasons.push("COIN POINTS DID NOT MATCH "+checkCoinPoints+" / "+finalGameData.totalCoinPoints);
    isValid=false;
  ...
  var checkPushPoints = 0;

  for(var i=0; i<breadcrumbs.length; i++){

    checkPushPoints += breadcrumbs[i].levelPushPoints;

  }

  if(checkPushPoints!==finalGameData.totalPushPoints){

    reasons.push("PUSH POINTS DID NOT MATCH "+checkPushPoints+" / "+finalGameData.totalPushPoints);
    isValid=false;
  ...
  var checkRightPoints = 0;

  for(var i=0; i<breadcrumbs.length; i++){

    checkRightPoints += breadcrumbs[i].levelRightPoints;

  }

  if(checkRightPoints!==finalGameData.totalRightPoints){

    reasons.push("RIGHT POINTS DID NOT MATCH "+checkRightPoints+" / "+finalGameData.totalRightPoints);
    isValid=false;
  ...
  var checkWrongPoints = 0;

  for(var i=0; i<breadcrumbs.length; i++){

    checkWrongPoints += breadcrumbs[i].levelWrongPoints;

  }

  if(checkWrongPoints!==finalGameData.totalWrongPoints){

    reasons.push("WRONG POINTS DID NOT MATCH "+checkWrongPoints+" / "+finalGameData.totalWrongPoints);
    isValid=false;
  ...
  var checkPenaltyPoints = 0;

  for(var i=0; i<breadcrumbs.length; i++){

    checkPenaltyPoints += breadcrumbs[i].levelPenaltyPoints;

  }

  if(checkPenaltyPoints!==finalGameData.totalPenaltyPoints){

    reasons.push("PENALTY POINTS DID NOT MATCH "+checkPenaltyPoints+" / "+finalGameData.totalPenaltyPoints);
    isValid=false;

  }

  // ---------------------------------------------------------------------------------------------
  // check if certain things are too high
  // ---------------------------------------------------------------------------------------------

  if(finalGameData.totalLifeBonus > 1000){

    reasons.push("LIFE BONUS IMPOSSIBLY HIGH "+finalGameData.totalLifeBonus+" / 1000");
    isValid=false;

  }

  if(finalGameData.totalCoinPoints > 5000){

    reasons.push("TOTAL COIN BONUS IMPOSSIBLY HIGH "+finalGameData.totalCoinPoints+" / 5000");
    isValid=false;

  }

  // ---------------------------------------------------------------------------------------------
  // check if life was ever increased
  // ---------------------------------------------------------------------------------------------

  for(var i=1; i<breadcrumbs.length; i++){

    // console.log("compare life: "+breadcrumbs[i].life +" / "+ breadcrumbs[i-1].life);

    if(breadcrumbs[i].life > breadcrumbs[i-1].life){

      reasons.push("LIFE WAS INCREASED AT SOME POINT ("+i+")");
      isValid=false;
